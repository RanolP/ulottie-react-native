...
export function bTransform(el, S, a, ctx, at) {
  const p = resolve(S[a], ctx, at);
  const an = resolve(S[a + 1], ctx, at);
  const s = resolve(S[a + 2], ctx, at);
  const rot = resolve(S[a + 3], ctx, at);
